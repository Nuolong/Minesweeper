  ...
  public Minesweeper(int s, int m){
    ...
    while(a < m){ //WHILE LOOP FOR MINES ADDED TO FIELD
      ...
      if (arrans[x][y] != -1) {
        ...
        if (x > 0 && y > 0 && arrans[x-1][y-1] != -1) arrans[x-1][y-1] += 1;
        if (x > 0 && arrans[x-1][y] != -1) arrans[x-1][y] += 1;
        if (x > 0 && y < arrans[0].length - 1 && arrans[x-1][y+1] != -1) arrans[x-1][y+1] += 1;
        if (y > 0 && arrans[x][y-1] != -1) arrans[x][y-1] += 1;
        if (y < arrans[0].length - 1 && arrans[x][y+1] != -1) arrans[x][y+1] += 1;
        if (x < arrans.length - 1 && y > 0 && arrans[x+1][y-1] != -1) arrans[x+1][y-1] += 1;
        if (x < arrans.length - 1 && arrans[x+1][y] != -1) arrans[x+1][y] += 1;
        if (x < arrans.length - 1 && y < arrans.length - 1 && arrans[x+1][y+1] != -1) arrans[x+1][y+1] += 1;
        ++a;
      }
    }
...
//-------------------------------------------------------//        
  ...
  public boolean isFinished(){
    ...
    for(int k = 0; k < arrresult.length; k++){
      for(int j = 0; j < arrresult.length; j++){
        if(arrresult[k][j] == "-" || arrresult[k][j].equals("!")){++count;}
      }
    }
    return count==mines;
  }
